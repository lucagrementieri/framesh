...
def test_flare_lrf(lrf_input: str, request: pytest.FixtureRequest) -> None:
    ...
    axes = framesh.flare.flare_lrf(mesh, vertex_index, radius=3.0, z_radius=1.0)
    assert np.allclose(np.dot(axes.T, axes), np.eye(3))
    assert np.isclose(np.linalg.det(axes), 1.0)

    none_z_radius_axes = framesh.flare.flare_lrf(mesh, vertex_index, radius=3.0)
...
def test_flare_frames(lrf_input: str, request: pytest.FixtureRequest) -> None:
    ...
    frames = framesh.flare.flare_frames(mesh, test_indices, radius=3.0, z_radius=1.0)
    for frame, vertex_index in zip(frames, test_indices, strict=True):
        single_frame = framesh.flare.flare_lrf(mesh, vertex_index, radius=3.0, z_radius=1.0)
        assert np.allclose(frame, single_frame)

    none_z_radius_frames = framesh.flare.flare_frames(mesh, test_indices, radius=3.0)
    for frame, vertex_index in zip(none_z_radius_frames, test_indices, strict=True):
        single_frame = framesh.flare.flare_lrf(mesh, vertex_index, radius=3.0)
        assert np.allclose(frame, single_frame)
